...
export default async function DossierPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* ─── 6. Gallery ───────────────────────────────────────────────── */}
      {(() => {
        const galleryImages = allImages.length > 0 ? allImages.map((img) => ({ url: img.url, alt: img.caption ?? dev.name })) : [
          { url: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
          { url: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
          { url: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
          { url: "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
          { url: "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
          { url: "https://images.unsplash.com/photo-1580587771525-78b9dba3b914?w=900&h=600&fit=crop&auto=format&q=80", alt: dev.name },
        ];
        return (
            ...
            <div className="container-padded">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {galleryImages.map((img, i) => (
                  <div key={i} className="relative h-52 overflow-hidden bg-navy/5">
                    <Image
                      src={img.url}
                      alt={`${img.alt} ${i + 1}`}
                      fill
