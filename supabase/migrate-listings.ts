...
async function migrate() {
  ...
  if (delErr) {
    console.error('Failed to delete existing developments:', delErr)
    process.exit(1)
  }
  ...
  for (const dirName of listingDirs) {
    ...
    try {
      ...
      console.warn(`⚠ Skipping ${dirName} — no JSON file found`)
      skipCount++
      continue
    }
    ...
    if (devUpsertErr) {
      console.warn(`\n  ⚠ Developer upsert failed: ${devUpsertErr.message}`)
    }
    ...
    // Upsert development (safe to re-run)
      ...
      .upsert(
        {
          ...
          search_price_max:
            pricing.search_price_max != null ? pricing.search_price_max * 100 : null,
    ...
    // Insert development_images — hero first, then gallery
    ...
    if (imageRecords.length > 0) {
      ...
      if (imgErr) console.warn(`\n  ⚠ Image insert error: ${imgErr.message}`)
    }
    ...
    const floorPlans = (config.floor_plans ?? []).map((fp: Record<string, unknown>) => ({
      ...
      plan_type: null,
      config:
        fp.beds != null && fp.bath != null ? `${fp.beds} bed, ${fp.bath} bath` : null,
    ...
    if (floorPlans.length > 0) {
      ...
      if (fpErr) console.warn(`\n  ⚠ Floor plan insert error: ${fpErr.message}`)
    }

    console.log(`✅ (${imageRecords.length} images, ${floorPlans.length} floor plans)`)
    successCount++
  }
