/**
 * Single source of truth for Off The Plan's social media URLs + icon
 * paths. Consumed by both the website footer (JSX) and the transactional
 * email templates (HTML strings), so the two stay in sync.
 *
 * Icon glyphs are stored as SVG path data only — wrapping
 * <svg viewBox="0 0 24 24" fill="currentColor"> happens at the call
 * site (a React component for the footer, a static HTML string for the
 * email helper below).
 */
...
export interface SocialLink {
  label: string;
  href: string;
  /** Content of the SVG `d=""` attribute, viewBox 0 0 24 24. */
  svgPath: string;
}
...
const BY_LABEL: Record<string, SocialLink> = Object.fromEntries(
 ...
 */
export function socialRowHtml(labels: string[], size = 20): string {
  const items = labels
    .map((l) => BY_LABEL[l])
